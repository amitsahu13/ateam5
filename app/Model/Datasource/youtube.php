...
class Youtube extends DataSource{
	...
	private function __buildUrl($key, $type, $options = null){
		switch ($type) {
			...
			$url.= $key.'/'.$feed.'/responses?v='.$this->config['api_version'];
			break;
	...
	public function findById($id = null){
		...
		@$jsondata = json_decode(file_get_contents($video_url."&alt=json"));
	...
	private function __cleanYoutubeId($subject){
		if(!strpos($subject,"www.youtube.com")){
			$subject = "http://www.youtube.com/watch?v=".$subject."&alt=json";
		}
