...
class PagesController extends AppController {
	...
	public function index($type=null){
		...
		$imageArray = $this->Slider->find('all',array('order'=>array('Slider.id'=>'DESC'),'limit'=>'3'));
	...
	{
		$this->layout = 'lay_page_view';
		$data=$this->Page->find('first',array('conditions'=>array('Page.slug'=>$this->params['static'])));
		
		$parent_view = $this->get_parent_view($data['Page']['parent_id']);
		 
		
		$this->set(compact('data','parent_view'));
	}
	
	function get_parent_view($id){
		
		if($id == 1){
			$content ="How it work?";
		}
		if($id == 2){
			$content ="About A Team 4 A Dream";
		}
		if($id == 3){
			$content ="Help";
		}
