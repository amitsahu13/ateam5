...
class ViolationsController extends AppController {
	...
	public function admin_index($defaultTab='All') {
		...
		$first_name = Sanitize::escape($this->request->data[$model]['first_name']);
		...
		array(
				...
				'recursive'=>1
						...
						$this->set('title_for_layout',  __('Violations', true));
	...
	public function admin_user_violation_list($type=null,$id=null) {
		$defaultTab='All';
		$model = $this->model;
		if(!isset($this->request->params['named'][$model])){
			$this->Session->delete('AdminSearch');
		...
		array(
            ...
            "$model.from_user_id" => $id,
				...
				'recursive'=>1
						...
						$this->set('title_for_layout',  __('Violation', true));
						...
						if($this->request->is('ajax')){
							$this->render('ajax/admin_user_violation_list');
						}else{
							$tabs = array('All'=>count($data));
							$this->set(compact('tabs'));
						}
