<?php
/**
 * RatingParameters Controller
 *
 * PHP version 5.4
 *
 */
class RatingParametersController extends AppController {
	/**
	 * RatingParameters name
	 *
	 * @var string
	 * @access public
	 */
	var	$name	=	'RatingParameters';
	var	$uses	=	array('RatingParameter');
	var $helpers = array('Html','General');
	var $model='RatingParameter';
	var $controller='rating_parameters';
	/*
	 * beforeFilter
	 * @return void
	 */
	...
	public function admin_index($defaultTab='All') {
		...
		if(!empty($this->request->data)){
			...
			if(isset($this->request->data[''.$model.'']['type']) && $this->request->data[''.$model.'']['type']!=''){
				$type = Sanitize::escape($this->request->data[''.$model.'']['type']);
				$this->Session->write('AdminSearch.type', $type);
			}
