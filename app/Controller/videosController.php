		...
		if ($this->request->is('post') || $this->request->is('put')) {
			if(!empty($this->request->data)) {
				...
				if ($this->$model->validates()) {
					...
					if(!empty($this->request->data[$model]['embeded_video'])){
						$this->request->data[$model]['embeded_video'] =  preg_replace($pattern, 'width="231" height="173"', $this->request->data[$model]['embeded_video']);
					}
					if ($this->$model->save($this->request->data)) {
						$this->Session->setFlash(__('The '.$model.' has been saved successfully',true), 'admin_flash_success');
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			if(!empty($this->request->data)) {
				...
				if ($this->$model->validates()) {
					...
					if ($this->$model->save($this->request->data)) {
						$this->Session->setFlash(__('The '.$model.' has been updated successfully',true), 'admin_flash_success');
						$this->redirect(array('action' => 'index'));
					} else {
						$this->Session->setFlash(__('The '.$model.' could not be saved. Please, try again.',true), 'admin_flash_error');
					}
