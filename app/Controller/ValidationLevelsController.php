<?php
/**
 * ValidationLevels Controller
 *
 * PHP version 5.4
 *
 */
class ValidationLevelsController extends AppController {
	/**
	 * JobFors name
	 *
	 * @var string
	 * @access public
	 */
	var	$name	=	'ValidationLevels';
	var	$uses	=	array('ValidationLevel');
	var $helpers = array('Html','General');
	var $model='ValidationLevel';
	var $controller='validation_levels';
	/*
	 * beforeFilter
	 * @return void
	 */
	...
	*/
	public function admin_add() {
		$model=$this->model;
		$controller=$this->controller;
		$this->Session->setFlash(__('UnAuthorized Access.'), 'admin_flash_error');
		$this->redirect(array('action'=>'index'));
		if ($this->request->is('post')) {
			if(!empty($this->request->data)) {
				if (!isset($this->request->params['data']['_Token']['key']) || ($this->request->params['data']['_Token']['key'] != $this->request->params['_Token']['key'])) {
				...
				$this->$model->set($this->request->data);
				$this->$model->setValidation('admin');
				$this->request->data[$model]['slug'] = strtolower(str_replace(' ','_',$this->request->data[$model]['name']));
				if ($this->$model->validates()) {
					if ($this->$model->save($this->request->data)) {
