<?php
/**
 * ProjectVisibilities Controller
 *
 * PHP version 5.4
 *
 */
class ProjectVisibilitiesController extends AppController {
	/**
	 ...
	 * @access public
	 */
	var	$name	=	'ProjectVisibilities';
	var	$uses	=	array('ProjectVisibility');
	var $helpers = array('Html','General');
	var $model='ProjectVisibility';
	var $controller='project_visibilities';
	/*
	 * beforeFilter
	 * @return void
	 */
	...
	public function admin_index($defaultTab='All') {
		...
		$this->paginate = array(
			''.$model.''=>array(	
				...
				'order'=>array(''.$model.'.order_key'=>'ASC'),
				'conditions'=>$filters,
				'recursive'=>1
		)
		);
	...
	*/
	...
	public function admin_updateorder(){
		...
		$controller=$this->controller;
		if(!empty($this->request->data)){
			$orderarray = explode(',',implode(',',$this->data));
			foreach($orderarray as $k=>$v){
				$myarrary = array();
				$myarrary[$model]['id'] = $v;
				$myarrary[$model]['order_key'] = $k+1;
				$insertarr[] =  $myarrary;
			}
			//pr($insertarr); die;
			$this->$model->saveAll($insertarr);
		}
