...
class TimeHelper extends AppHelper {
	...
	public function niceShort_new($dateString = null, $userOffset = null) {
		...
		if ($this->isToday($dateString, $userOffset)) {
			...
			$format = $this->convertSpecifiers("%b %e,{$y}", $date);
	...
	public function rShort($dateString = null, $userOffset = null) {
		$date = $dateString ? $this->fromString($dateString, $userOffset) : time();
		...
		if ($this->isToday($dateString, $userOffset)) {
			...
			$format = $this->convertSpecifiers("%b %eS{$y}", $date);
			$ret = $this->_strftime($format, $date);
		}
	...
	public function R_Short($dateString = null, $userOffset = null) {
		$date = $dateString ? $this->fromString($dateString, $userOffset) : time();
	...
	public function getDateInDetail($date){
		if($date!=''){
			$str = strtotime($date);
			$now = time();
			if($str < $now){
			...
			$forHour = 	$datediff%(60*60*24);
			$totHour =  floor($forHour/(60*60));
			$totday  =  floor($datediff/(60*60*24));
			if($totday < 1 && $totHour < 1){
				$min = floor($datediff/(60));
				return $min.' min ago';
			}else{
				return $totday.'d, '.$totHour.'h';
			}
		}else{
			return '0d, 0h';
		}
