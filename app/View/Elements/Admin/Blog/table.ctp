...
<table>
	...
	<thead>
		<tr>	
			...
			<th><?php echo ($this->ExPaginator->sort('Blog.title', 'Blog Title'))?></th>
			<th><?php echo ($this->ExPaginator->sort(''.$model.'.status', 'Status'))?></th>
			<th><?php echo ($this->ExPaginator->sort('Blog.created', 'Created'))?></th>
	...
	<tbody>
		...
		<tr <?php echo ($alt==0)?'class="alt-row"':''; $alt=!$alt;?>>
			...
			<b>
				<?php 
					//echo $this->General->wrap_long_txt($value['Blog']['title'],0,50);
				?>
				<?php echo ($this->Html->link($this->General->wrap_long_txt($value['Blog']['title'],0,50),array('action'=>'view', $value['Blog']['id']),array('title'=>'View Details')));?>
			</b></td>	
			...
			<td><?php echo ($this->Time->niceShort($value['Blog']['created']));?></td>			
			<td>
				<!-- Icons -->
				 <?php 
