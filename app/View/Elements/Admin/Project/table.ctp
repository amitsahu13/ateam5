...
<table>				
	<thead>
		<tr>
			...
			<th><?php echo ($this->ExPaginator->sort('User.username', 'User'))?></th>
			<th><?php echo ($this->ExPaginator->sort('Category.name', 'Category'))?></th>
			<th><?php echo ($this->ExPaginator->sort('ProjectStatus.name', 'Project Level Status'))?></th>
			<th><?php echo ($this->ExPaginator->sort('Project.status', 'Status'))?></th>
	...
	<tbody>
		...
		<tr <?php echo ($alt==0)?'class="alt-row"':''; $alt=!$alt;?>>
			...
			<td><b><a href="javascript:void(0);"><?php echo $value[''.$model.'']['title'];?></a> <?php //echo $this->Html->link($value[''.$model.'']['title'],array('controller'=>'projects','action'=>'view',$value[''.$model.'']['id']));?></b></td>
			<td><b><?php echo $value['User']['username'];?></b></td>
			<td><b><?php echo $value['Category']['name'];?></b></td>
			<td><?php echo $value['ProjectStatus']['name'];?></td>	
