...
<table>
	...
	<thead>
		<tr>
			...
			<th><?php echo ($this->ExPaginator->sort('Category.type_for', 'Category For'))?></th>
			<th><?php echo ($this->ExPaginator->sort('Category.parent_id', 'Parent Category'))?></th>
	...
	<tfoot>
		<tr>
			<td colspan="7">
				...
				echo $this->element('Admin/admin_pagination', array("paging_model_name"=>"Category", "total_title"=>"Categorys")); ?>
	...
	<tbody>
		...
		<tr <?php echo ($alt==0)?'class="alt-row"':''; $alt=!$alt;?>>
			...
			<td>
				<?php
					if($value['Category']['type_for']==1)
					{
						echo "Project";
					}
					elseif($value['Category']['type_for']==2)
					{
						echo "Job";
					}
					else
			...
			<td><?php echo ($value['Parent']['name']?$value['Parent']['name']:'&ndash;');?></td>
			<td><?php echo ($this->Html->link($this->Layout->Status($value['Category']['status']), array('action'=>'status',$value['Category']['id'],'token'=>$this->params['_Token']['key']), array('title'=>$value['Category']['status']==1?'deactivate':'activate')));?></td>
			<td>
				<!-- Icons -->
				 <?php 
